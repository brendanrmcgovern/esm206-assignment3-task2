...
-- Row 49 removed "Mundi" because I don't have a third name column. 
...
mpg_make_model <- mpg %>% # Start with mpg
  unite(col = make_model, # Create a new column called 'make_model'
        ...
        sep = " mo money mo problems " # Puts "HEY!" between the pieces (default is underscore)
        ) 
...
some_dates <- c(06271989, 12221990, 03221959, 06241955)
...
hooks_by_month <- gasol_hookshots %>% 
  count(month) %>% 
  rename(hooks_attempted = n)
